...
<%
	...
	String dbUser = "root";
	String dbPass = "1234";
	String query0 = "SELECT * FROM operation WHERE id = " + id;
	...
	String deptStatus = result.getString("deptStatus");
	String arvStatus = result.getString("arvStatus");
	String updateTime = result.getString("updateTime");
	String trainId = result.getString("trainId");
	String engineerId = result.getString("engineerId");
	String trainCodeId = result.getString("trainCodeId");
%>
<div class="regist">
	...
	<div class="registform">
		<form action="OperationModifyAction.jsp" method="post">
			ID<br>
			...
			<select name="deptStatus">
				<option>--선택하시오--</option>
				<option value="0" <% if (deptStatus.equals("0")) {%>selected<%}%>>준비 중</option>
				<option value="1" <% if (deptStatus.equals("1")) {%>selected<%}%>>정시 출발</option>
				<option value="2" <% if (deptStatus.equals("2")) {%>selected<%}%>>지연 출발</option>
			</select>
			Arrival Status<br>
			<select name="arvStatus">
				<option>--선택하시오--</option>
				<option value="0" <% if (arvStatus.equals("0")) {%>selected<%}%>>출발 전</option>
				<option value="1" <% if (arvStatus.equals("1")) {%>selected<%}%>>운행 중</option>
				<option value="2" <% if (arvStatus.equals("2")) {%>selected<%}%>>정시 도착</option>
				<option value="3" <% if (arvStatus.equals("3")) {%>selected<%}%>>지연 도착</option>
			</select>
			Update Time<br>
			<input name="updateTime" type="text" autocomplete="off" value="<%=updateTime%>">
			Train ID<br>
			<div class="choose">
				<input name="train" id="train" type="text" autocomplete="off" value="<%=trainId%>" readonly>
				...
				<div id="popinfo" class="modal">
					<div class="modal-content">
						...
						<%
	             			String query = "SELECT trainid FROM operation";
			...
			TrainCode ID
			<div class="choose">
				<input name="trainCode" id="trainCode" type="text" autocomplete="off"  value="<%=trainCodeId%>" readonly>
