...
<div class="menu">
	...
	<div class="menulist">
		...
		<button type="button" id="Train" class="menubuttonactive" onClick="change_btn(event); location.href='Train.jsp'">기차 관리</button><br>
...
<div class="main">
	...
	<div class="goregist">
		기차 추가<button type="button" onClick="location.href='TrainRegist.jsp'">+</button>
	</div>
	<%
		...
		if (searchSelect == "id" || searchSelect == "status") {
			query = "SELECT * FROM train WHERE " + searchSelect + "=" + keyword;	
		}
		else {
			query = "SELECT * FROM train WHERE " + searchSelect + " like '%" + keyword + "%'";	
		}
	...
	for (i=1; i<rowList.length; i++) {    
		...
		row.onclick = function(){ 
			return function(){ 
				...
				location.href='TrainModify.jsp?id=' + number;
			};
		}(row);
	}      
}
		...
		for (i=1; i<rowList.length; i++) {    
			...
			row.onclick = function(){ 
				return function(){ 
					...
					location.href='TrainDelete.jsp?id=' + number;
				};
			}(row);
		}    	
	}      
