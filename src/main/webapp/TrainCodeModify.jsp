...
<%
	...
	String dbUser = "root";
	String dbPass = "1234";
	String query = "SELECT * FROM trainCode WHERE id = " + id;
	...
	String lineId = result.getString("lineId");
...
<div class="regist">
	...
	<div class="registform">
		<form action="TrainCodeModifyAction.jsp" method="post">
			ID<br>
			<input name="id" type="text" autocomplete="off" value="<%=Userid%>" readonly><br>
			Code<br>
			<input name="code" type="text" autocomplete="off"  value="<%=code%>"><br>
			Day<br>
			<select name="day">
				<option value="월요일" <% if (day.equals("월요일")) {%>selected<%}%>>월요일</option>
				<option value="화요일" <% if (day.equals("화요일")) {%>selected<%}%>>화요일</option>
				<option value="수요일" <% if (day.equals("수요일")) {%>selected<%}%>>수요일</option>
				<option value="목요일" <% if (day.equals("목요일")) {%>selected<%}%>>목요일</option>
				<option value="금요일" <% if (day.equals("금요일")) {%>selected<%}%>>금요일</option>
				<option value="토요일" <% if (day.equals("토요일")) {%>selected<%}%>>토요일</option>
				<option value="일요일" <% if (day.equals("일요일")) {%>selected<%}%>>일요일</option>
			</select>
			<br>
			Line<br>
			<div class="choose">
				<input name="lineId" id="lineId" type="text" autocomplete="off" value="<%=lineId%>" readonly>
				...
				<div id="popinfo" class="modal">
					<div class="modal-content">
             			...
             			<%
	             			String query1 = "SELECT id, name FROM line";
			...
			Start Station<br>
			<input name="startStation" type="text" autocomplete="off" value="<%=startStation%>"><br>
			Start Time<br>
			<input name="startTime" type="text" autocomplete="off" value="<%=startTime%>"><br>
			End Station<br>
			<input name="endStation" type="text" autocomplete="off" value="<%=endStation%>"><br>
			End Time<br>
			<input name="endTime" type="text" autocomplete="off" value="<%=endTime%>"><br>
			<button>수정하기</button>
		</form>
	</div>
</div>
