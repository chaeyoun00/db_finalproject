...
<%
	...
	if (keyword == "") {
		...
		script.println("location.href='Station.jsp'");
		script.println("</script>");
	}
%>
<div class="menu">
	...
	<div class="menulist">
		...
		<button type="button" id="Station" class="menubuttonactive" onClick="change_btn(event); location.href='Station.jsp'">역 관리</button><br>
...
<div class="main">
	<div class="search">
		<form action="StationSearch.jsp" method="post">
			<select name="searchSelect">
				...
				<option value="headquarterId">Headquarter</option>
			</select>
	...
	<div class="goregist">
		역 추가<button type="button" onClick="location.href='StationRegist.jsp'">+</button>
	</div>
	<%
		...
		if (searchSelect == "id" || searchSelect == "budget") {
			query = "SELECT * FROM station WHERE " + searchSelect + "=" + keyword;	
		}
		else {
			query = "SELECT * FROM station WHERE " + searchSelect + " like '%" + keyword + "%'";	
		}
	...
	<div class="inform">
		...
		<tr align="center">
			...
			<td><%=result.getString("headquarterId") %></td>
			<td>
	...
	for (i=1; i<rowList.length; i++) {    
		...
		row.onclick = function(){ 
			return function(){ 
				...
				location.href='StationModify.jsp?id=' + number;
			};
		}(row);
	}      
}
		...
		for (i=1; i<rowList.length; i++) {    
			...
			row.onclick = function(){ 
				return function(){ 
					...
					location.href='StationDelete.jsp?id=' + number;
				};
			}(row);
		}    	
	}      
