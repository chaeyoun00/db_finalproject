...
<div class="main">
	<div class="search">
		<form action="EngineerSearch.jsp" method="post">
	...
	<div class="goregist">
		엔지니어 추가<button type="button" onClick="location.href='EngineerRegist.jsp'">+</button>
	</div>
	<%
		...
		String dbPass = "1234";
		String query = "";
		if (searchSelect == "id" || searchSelect == "status") {
			query = "SELECT * FROM engineer WHERE " + searchSelect + "=" + keyword;	
		}
		else {
			query = "SELECT * FROM engineer WHERE " + searchSelect + " like '%" + keyword + "%'";	
		}
	...
	for (i=1; i<rowList.length; i++) {    
		...
		row.onclick = function(){ 
			return function(){ 
				...
				location.href='EngineerModify.jsp?id=' + number;
			};
		}(row);
	}      
}
		...
		for (i=1; i<rowList.length; i++) {    
			...
			row.onclick = function(){ 
				return function(){ 
					...
					location.href='EngineerDelete.jsp?id=' + number;
				};
			}(row);
		}    	
	}      
